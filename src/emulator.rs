...
t3   {:016x} t4 {:016x} t15 {:016x} t6  {:016x}
pc   {:016x}"#,
...
//            print!("{}\n\n", self);
    ...
    /// Compile a JIT function for `pc` until all paths lead to
...
"#, pc.0);
        ...
        let cachename = Path::new("jitcache")
            .join(format!("{:x?}", proghash));

        // If we aren't the first to access the cache, idle loop until
        // the first person has compiled the code
        if !first {
